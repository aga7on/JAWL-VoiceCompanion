...
class GraphDB:
    ...
    def __init__(self, db_path: str) -> None:
        ...
        # Some platforms install an import-compatible kuzu stub.  Keep the
        # process alive, but never let that stub report successful mutations.
    ...
    async def connect(self) -> None:
        def _init_db() -> None:
            ...
            self._init_schema()
            self.backend_available = self._probe_backend()
        ...
        try:
            ...
            if self.backend_available:
                ...
                main_logger.warning(
                    "[Graph DB] Backend is unavailable; graph skills are disabled."
                )
    ...
    def _probe_backend(self) -> bool:
        """Distinguish a working Kuzu connection from an import-only stub."""

        if not self.conn:
            return False
        try:
            result = self.conn.execute("RETURN 1")
            available = bool(result.has_next())
            while result.has_next():
                result.get_next()
            return available
        except Exception as exc:
            main_logger.warning(f"[Graph DB] Backend probe failed: {exc}")
            return False
