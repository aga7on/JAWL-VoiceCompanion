...
class GraphCRUD:
    ...
    def _get_all_names(self) -> List[str]:
        ...
        if not self.db.conn or not getattr(self.db, "backend_available", True):
            return []
    ...
    async def add_concept(
        ...
        if not getattr(self.db, "backend_available", True):
            return SkillResult.fail("Graph DB backend is unavailable.")
