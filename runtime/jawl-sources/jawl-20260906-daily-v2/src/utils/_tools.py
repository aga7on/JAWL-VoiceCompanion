...
def dump_prompt_to_file(filename: str, messages: list, meta_header: str = "") -> None:
    ...
    try:
        ...
        # Keep diagnostic dumps inside the active isolated instance. Several
        # callers pass historical relative names such as logs/prompts/*;
        # resolving them against the source tree would mutate a pinned source
        # snapshot and invalidate its manifest on first boot.
        if not file_path.is_absolute():
            log_dir = os.environ.get("JAWL_LOG_DIR", "").strip()
            if log_dir:
                file_path = Path(log_dir) / file_path
