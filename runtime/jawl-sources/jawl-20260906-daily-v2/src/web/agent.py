...
CRASH_LOG = INSTANCE_PATHS.log_dir / "startup" / "startup_error.log"
...
SUPERVISED = os.getenv("JAWL_SUPERVISED", "").strip().lower() in {
    "1", "true", "yes"
}
...
def _supervised_manager():
    """Return the native lifecycle owner selected by the launcher."""
...
def start() -> Dict[str, Any]:
    ...
    if SUPERVISED:
        ...
        try:
            manager = _supervised_manager()
            manager.request_start(INSTANCE_PATHS.instance_id)
            supervisor = manager.ensure_supervisor()
        except Exception as exc:  # noqa: BLE001 - expose bounded operator error
            return {
                "ok": False,
                "error": "native instance supervisor could not start the agent",
...
def stop(timeout: int | None = None) -> Dict[str, Any]:
    ...
    if SUPERVISED:
        try:
            # Revoke the durable desired state before looking at the child PID.
            # During the startup race the PID file may not exist yet; returning
            # early in that window would leave the supervisor authorized to
            # relaunch the profile after the launcher has already failed.
            _supervised_manager().request_stop(INSTANCE_PATHS.instance_id)
        except Exception as exc:  # noqa: BLE001 - keep the stop fail-closed
            return {
                "ok": False,
                "error": "native instance supervisor could not accept stop",
                "detail": f"{type(exc).__name__}: {str(exc)[:300]}",
            }
