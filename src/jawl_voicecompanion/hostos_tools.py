...
class HostOSExecutor:
    ...
    _idempotency: dict[str, tuple[str, dict[str, Any], float]] = field(
        default_factory=dict, repr=False
    )
    _idempotency_lock: threading.Lock = field(default_factory=threading.Lock, repr=False)
    _max_idempotency_records: int = field(default=256, repr=False)
    ...
    def execute(self, request: ToolRequest, has_approval: bool = False) -> dict[str, Any]:
        ...
        if spec.mutating:
            # request_id is server-generated by default and is therefore a
            # safe fallback key for callers that do not provide a separate
            # idempotency key. Explicit retries should reuse that key.
            key = str(request.idempotency_key or request.request_id).strip()
            fingerprint = self._mutation_fingerprint(canonical_request)
            with self._idempotency_lock:
                previous = self._idempotency.get(key)
                if previous is not None:
                    old_fingerprint, old_result, _created = previous
                    if old_fingerprint != fingerprint:
                        return self._failure(
                            canonical_request,
                            "conflict",
                            "idempotency_key_reused_for_different_mutation",
                        )
                    return copy.deepcopy(old_result)
                result = self._dispatch_result(canonical_request, decision)
                if result.get("status") not in {"denied", "failed", "timeout"}:
                ...
                return result
        return self._dispatch_result(canonical_request, decision)

    def _dispatch_result(
        self, request: ToolRequest, decision: ToolDecision
    ) -> dict[str, Any]:
        try:
            result = self._dispatch(request)
        except PermissionError as exc:
            return self._failure(request, "denied", str(exc))
        except (OSError, ValueError, subprocess.SubprocessError) as exc:
            return self._failure(request, "failed", str(exc)[:500])
        return {
            "schema_version": 1,
            "request_id": request.request_id,
    ...
    @staticmethod
    def _mutation_fingerprint(request: ToolRequest) -> str:
        payload = {
            "tool": request.tool,
    ...
    def _prune_idempotency(self) -> None:
        ...
        for key, _item in sorted(
            self._idempotency.items(), key=lambda item: item[1][2]
        )[: len(self._idempotency) - self._max_idempotency_records]:
    ...
    def _delete_file(self, request: ToolRequest) -> dict[str, Any]:
        ...
        permanent = request.arguments.get("permanent", False)
        if not isinstance(permanent, bool):
            raise ValueError("permanent must be boolean")
        if permanent:
            path.unlink()
            return {"path": str(path), "deleted": True, "recoverable": False}
        quarantine = path.parent / f".jawl-trash-{uuid.uuid4().hex}-{path.name}"
    ...
    def _terminate_process(process: subprocess.Popen, *, force: bool = False) -> None:
        """Stop an owned process and its descendants where the OS permits it."""
