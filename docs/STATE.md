...
## UI-поворот (ADR-041) — 2026-09-19

Переход от control-center к интерфейсу жизни с компаньоном. Зафиксировано
ADR-041: Product Core поверх engines — одна программа с каноническими
сущностями (Conversation/Task/Memory/Perception/Presence/Character/Runtime),
движки = реализации, физические границы процессов сохраняются как изоляция,
но управляются runtime supervisor и невидимы пользователю.

- **E1**: 4 раздела (Компаньон/Задачи/Память/Настройки), жизни-rail
  (состояние · восприятие · доступ · ⛔), бренд «Компаньон».
- **E2**: home 60/40 сцена/диалог, stat-grid и dashboard-язык убраны.
- **E3**: Задачи — live-планы из журнала (действия + ✓/✗), real data.
- **E4**: Память — человекочитаемые фразы + «Подробнее» (provenance свернут);
  журнал автономности перенесён в Задачи. **Найден баг**: null `#level`
  после удаления старой access-панели обрывал refresh (память не рендерилась);
  добавлены null-guards.
- **E5**: единая вкладка Настройки с подразделами (Агент/Голос и
  восприятие/Доступ/Диагностика); старые панели voice/perception/access/
  system/jawl удалены; проверено отсутствие дублей id в DOM.
- **E6**: мобильная bottom nav (4 колонки, fixed), планшет/десктоп sidebar.
- Все срезы live-проверены в headless Edge (навигация, подразделы, задачи,
  память, mobile 375px, 0 console-errors).

Осталось (следующие шаги): runtime supervisor + manifest вместо хардкод-путей
Rust-клиента (этап 2 ADR-041 / аудит), затем Product API-фасады (этап 1).

## Goal round 3 — 2026-09-19: окончательный вердикт по D1 (blendshapes v5)

Все доступные пути к pure-Rust blendshapes исчерпаны и задокументированы:

1. **Mocari GitHub master = crates.io 0.4.0** (последний коммит 13.07.2026,
   тот же день, что релиз): blendshape-шаги только в enum `ModelUpdateStep`,
   парсера и вызова `apply_art_mesh_blend_shape_delta` нет ни в одной ветке.
2. **moc2cmo 1.1.18** (единственный публичный Rust-парсер v5): 28 упоминаний
   "blend" — все про PSD-blend/ExpressionBlend, blendshape-деформацию v5 не
   реализует; новые секции 102–151 не читает.
3. **Бинарный реверс** (мой анализ): 50 новых v5-секций размечены по размерам
   (sec 108–113: по 8832 B — кандидаты на per-drawable blendshape-данные,
   260 drawables × ~34 B), но семантика без ground truth не восстанавливается;
   содержимое не сходится с индексами drawables напрямую. Дни работы, риск
   неверной интерпретации без возможности валидации.
4. **Официальная страница sample-моделей Live2D подтверждает диагноз**:
   mao_pro = "Niziiro Mao", учебная модель именно по **Blend Shapes** (Cubism
   5.3 фича) — её рот принципиально на blendshapes, не на классических
   keyform-деформациях.
5. **Cubism Core DLL** найден только внутри сторонней игры в Downloads
   (0.23 MB, без версии) — использовать чужую игровую поставку проприетарной
   DLL как зависимость продукта некорректно; официальная поставка требует
   принятия лицензий владельцем (Cubism SDK for Native, ручное скачивание).

**Итог**: mao_pro в pure-Rust без blendshapes невозможен. Два честных варианта:
(а) владелец принимает Cubism SDK License Agreement и скачивает официальный
Cubism Core — тогда подключаем DLL через FFI (заготовка на libloading
обкатана, убрана до востребованности); (б) mao_pro остаётся на WebView2/
Cubism Core web-пути (уже работает, рот живой), а pure-Rust native-окно
использует Hiyori/v3-модели. Оба требуют решения владельца — агент его
принять не может (EULA).

Сборка клиента после чистки: `cargo build --release` EXIT 0.

## Ночная сессия 3 — 2026-09-19 (goal round 2, продолжение)

- **Lip-sync по реальной громкости (RMS)**: амплитуда рта теперь считается из
  PCM каждого TTS-чанка (`_pcm_rms`), а не константой 0.6. Юнит-проверка:
  синус 0.354, речь RMS 0.02/0.035/0.05/0.08 → рот 0.42/0.56/0.67/0.85
  (sqrt-компрессия в видимый диапазон). Тесты web 67/67. Коммит c793243+5d10db8.
- **Блокер live-проверки RMS**: стек, поднятый из agent-сессии, падает на
  startup (stop-file/heartbeat race из-за wrapper-cleanup между tool-вызовами)
  — 3 попытки за раунд, все неудачны. Live-подтверждение «рот visibly двигается
  по громкости» требует стека из окна пользователя + визуальной оценки.
  Сам код детерминированно проверен юнитом (та же функция на тех же байтах).
