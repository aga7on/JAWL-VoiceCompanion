...
Live local LLM step is COMPLETE: one accepted `--turns 1` run on owned JAWL
(`runtime/daily-live-64k.json`, `pass:true`, ~48 s). The real blocker was
context, not the loop: LM Studio loaded the model with `n_ctx 15872` while JAWL
inflates prompts to 16–35k → HTTP 400 `exceed_context_size_error`. Fixed by
`lms load ea07de5ddbf7bac67aee9db5d525e9ea830e9e0d --context-length 65536 --yes`
(now served as `...:2`, targeted by `config/jawl/settings.yaml`). Companion
hardening: adaptive context budget, `max_react_steps: 8`, `goal_mode: false`,
owned directive `config/jawl/prompts/custom/RESPOND_DIRECTLY.md`, snapshot patch
`scripts/patches/jawl-context-budget-companion.patch` (no namespace index, no
`SkillCatalog` in base prefixes). NEXT — Phase 3 P0-B connected loop: seed a fact
to `SQLNotes` via a live turn, restart the console, and confirm recall on the
same question; then add a native action, and re-run the synthetic live matrix
(Phase 4) end-to-end on the working provider.
